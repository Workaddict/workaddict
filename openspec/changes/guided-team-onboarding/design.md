...
- Token prefill parameters: **checked live on 2026-09-23** (task 1.2), signed in as the repository owner, with the URL `…/settings/personal-access-tokens/new?name=Workaddict&description=…&target_name=Team-Wels&expires_in=90&contents=write`. Result: **only `name` (and `description`) are applied.** The form opened on the **personal account**, not `Team-Wels`; expiration showed GitHub's default of **30 days**, not 90; **Permissions were empty**, so `contents=write` was ignored. This is worse than community discussion #188111 described: `target_name` does not even preselect the owner visually, so there was nothing to lose by re-picking it.
  **Decision (revised):** drop `target_name`, `expires_in` and `contents` from the link — sending parameters GitHub ignores promises a prefill the page does not deliver. `githubLinks.newToken` keeps only `name` and `description`, and its `owner` argument now only tells the checklist which owner to name. The checklist says up front that GitHub fills in the name only, and walks through the four fields the user must set: resource owner (switch it away from the personal account; it cannot be changed later), expiration (90 days or more, since GitHub proposes 30), repository access, and Contents → Read and write. The sudo confirmation stays in the list as step 1.
  If GitHub ever honours these parameters, re-adding them is a one-line change in `githubLinks.newToken` plus the checklist wording.

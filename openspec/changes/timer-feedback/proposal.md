...
- **Stop the timer when the page is closed** (new, per-device setting, **on by default**): the device that started the timer records a local heartbeat. When the app is opened again after all Workaddict tabs on that device were closed (a reload only after more than 2 minutes), it asks: "Stop at <close time>", "Keep running" or "Stop now". A hidden tab or a sleeping laptop does not count as closed. Timers started on another device are never touched. The actual stop is written on the next open, because a closing page cannot finish the GitHub writes.
...
- **Time format** (added after the first live test): clock times show as 24-hour by default, with a per-device option for 12-hour (AM/PM) in Settings. Time inputs follow it and accept either format.
...
- `app-shell`: "Settings page" lists the new "Stop timer when I close the page" and time format settings.
- `time-tracking` also gains "Time format".

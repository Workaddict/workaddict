...
The system SHALL list team members as the collaborators of the data repository, showing each member's GitHub login and avatar, and SHALL fall back to logins found in the repository's data files when the collaborator list is unavailable.

#### Scenario: Collaborators available
- **WHEN** the collaborator list can be read with the user's token
- **THEN** the system shows all collaborators as members

#### Scenario: Collaborators unavailable
- **WHEN** the collaborator request is denied
- **THEN** the system derives members from the logins under `entries/` and `timers/` and includes the current user
